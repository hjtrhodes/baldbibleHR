...
exports.createImage = (req, res, next) => {
  // call the post method, which adds a route to the router object, to handle POST requests to the /api/stuff endpoint
  ...
  const image = new Image({
    // create a new Image object, using the Image model
  ...
  image
    .save() // call the save method, which saves the Image object to the database
...
exports.modifyImage = (req, res, next) => {
  // call the put method, which adds a route to the router object, to handle PUT requests to the /:id endpoint
  ...
  if (req.file) {
    ...
    req.body.image = JSON.parse(req.body.image); // parse the request body
    image = {
      // set the image object to an object with the following properties
...
exports.deleteImage = (req, res, next) => {
  // call the delete method, which adds a route to the router object, to handle DELETE requests to the /:id endpoint
  Image.findOne({ _id: req.params.id }) // call the findOne method, which returns a promise, which resolves to the Image object with the specified id
    .then((image) => {
      // call the then method, which adds a callback function to the promise, to handle the success case
      if (!image) {
      ...
      if (image.userId !== req.auth.userId) {
        // if the userId property of the Image object does not match the userId property of the request body
      ...
      fs.unlink("Images/" + filename, () => {
        // call the unlink method, which deletes the file with the specified path
